...
# def cluster_deploy_request(job_id, instance_num, cluster_id):
#     cluster_ip = get_cluster_ip_by_id(cluster_id)
#     if cluster_ip is None:
#         print(f"Failed to get cluster IP for cluster {cluster_id}")
#         return

#     # delete this later
#     cluster_ip = "46.249.99.42"

#     request_address = f"http://{cluster_ip}:10105/api/calculate/deploy/{job_id}/{instance_num}"
#     try:
#         response = requests.post(request_address, headers={"Authorization": f"Bearer {token}"})
#         if response.status_code == 200:
#             print(f"Successfully sent deploy request for job {job_id}, instance {instance_num}")
...
def get_all_jobs():
    request_address = SYSTEM_MANAGER_ADDR + "/api/services"
    print("calling", request_address)
    try:
        response = requests.get(request_address, headers={"Authorization": f"Bearer {token}"})
        if response.status_code == 200:
